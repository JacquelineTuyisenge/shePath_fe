...
import { MessageSquare, Users, Settings, Menu, X, Edit2Icon, MessageCircle, Heart } from "lucide-react";
...
const MentorDashboard = () => {
  ...
  const learners = Array.isArray(chats)
    ? [...new Map(chats.filter((chat) => chat.receiver?.id === currentUser?.id).map((chat) => [chat.sender.id, chat.sender])).values()]
    : [];
    ...
    <div className="flex min-h-screen bg-light-background dark:bg-dark-background">
      ...
      >
        ...
        <div className="p-2 mt-4">
          <Link to="/" className="text-md font-bold text-light-primary mt-6 dark:text-dark-text dark:hover:text-light-secondary">
            Exit Dashboard
          </Link>
        </div>
      </aside>
...
{activeTab === "messages" && (
          <section className="w-full mt-12 p-6 dark:text-dark-text">
            <h1 className="text-2xl font-bold mb-6 text-center text-light-primary">Messages</h1>
            <div className="flex flex-col gap-6">
              ...
              <div className="flex flex-col lg:flex-row flex-1 md:gap-6">
                ...
                >
                  <ConversationsList onSelectPartner={setSelectedConversationPartner} selectedPartnerId={selectedConversationPartner} />
                </div>
        ...
        {activeTab === "learners" && (
          <section className="w-full mt-12 p-6 dark:text-dark-text">
            <h1 className="text-2xl font-bold mb-6 text-center text-light-primary">My Learners</h1>
            <div className="space-y-4">
              {learners.length > 0 ? (
                learners.map((learner) => (
                  ...
                  >
                    ...
                    <div className="flex-1">
                      <p className="text-lg font-semibold">{learner.firstName} {learner.lastName}</p>
                      <p className="text-sm text-gray-600 dark:text-gray-300">Reached out via chat</p>
                    </div>
                    <button
                      onClick={() => {
                ...
                <p className="text-gray-600 dark:text-gray-300 text-center">No learners have reached out yet.</p>
              )}
            </div>
          </section>
        )}
        ...
        {activeTab === "settings" && (
          <section className="w-full mt-12 p-6 dark:text-dark-text">
            <h1 className="text-2xl font-bold mb-4 text-center text-light-primary">Settings</h1>
